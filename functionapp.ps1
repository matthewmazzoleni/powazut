...
###APP CONFIGURATION####
########################
$appServiceConfigOptions = ConvertFrom-Json (Get-Content -Raw -Path "functionapp.config.json")
for ($i = 0; $i -lt $appServiceConfigOptions.Length; $i++) {
    Write-Host ("{0}: {1}" -f ($i + 1), $appServiceConfigOptions[$i].Name)
}

$appServiceConfigSelection = Read-Host "Seleziona il tipo di Function App"
...
$OS_TYPE = $appServiceConfigOptions[$i].OS_TYPE
$RUNTIME = $appServiceConfigOptions[$i].RUNTIME
$RUNTIME_VERSION = $appServiceConfigOptions[$i].RUNTIME_VERSION
$STORAGE_SKU = $appServiceConfigOptions[$i].STORAGE_SKU
...
$FUNCTION_VERSION = $appServiceConfigOptions[$i].FUNCTION_VERSION
$ALLOW_BLOB_PUBLIC_ACCESS = $appServiceConfigOptions[$i].ALLOW_BLOB_PUBLIC_ACCESS
$ALLOW_CROSS_TENANT_REPLICATION = $appServiceConfigOptions[$i].ALLOW_CROSS_TENANT_REPLICATION
...
$APPLICATION_NAME = Read-Host "Inserisci il nome alla function app"

$INSTANCE_NUMBER = Read-Host "Inserisci il numero di istanza (es: 001)"

$STORAGE_NAME = Read-Host "Inserisci il nome dello storage (es: ab fa p appname 001)"

$FUNCTIONAPP_NAME = $CUSTOMER_NAME + "-" + $APPLICATION_NAME + "-" + $RESOURCE_NAME_IDENTIFIER + "-" + $APPLICATION_TYPE + "-" + $LOCATION_SHORT + "-" + $INSTANCE_NUMBER
...
#storage
$functionAppStorage = New-AzStorageAccount -Name $STORAGE_NAME -Location $LOCATION -ResourceGroupName $RESOURCE_GROUP -SkuName $STORAGE_SKU -AllowBlobPublicAccess $ALLOW_BLOB_PUBLIC_ACCESS -AllowCrossTenantReplication $ALLOW_CROSS_TENANT_REPLICATION

# create the function app

$functionApp = New-AzFunctionApp -ResourceGroupName $RESOURCE_GROUP -Name $FUNCTIONAPP_NAME -OSType $OS_TYPE -Runtime $RUNTIME -RuntimeVersion $RUNTIME_VERSION -StorageAccountName $STORAGE_NAME -FunctionsVersion $FUNCTION_VERSION -PlanName $APP_SERVICE_PLAN

# sleep
Start-Sleep -Seconds 30

$webApp = Get-AzWebApp -ResourceGroupName $RESOURCE_GROUP -Name $FUNCTIONAPP_NAME
...
Update-AzTag -ResourceId $functionAppStorage.id -Tag $tags -Operation Merge
